...
void OpenGLVisitor::visit(Group* g) {
	...
	case EM_GL_GCOL: {
		...
		for ( ; shapeIter != shapeEnd; shapeIter++) {
			...
			if ((*shapeIter)->m_Texture != NULL && filter != -1) continue;
...
#if EM_VERTEXARRAY
			glVertexPointer(3, GL_FLOAT, sizeof(Vertex3D), (GLvoid*)(*shapeIter)->m_vVtxAlign.begin());
			glColorPointer(3, GL_FLOAT, sizeof(Color), (GLvoid*)(*shapeIter)->m_vLight.begin());
#endif
			...
			for ( ; polyIter != polyEnd; polyIter++) {
				...
				for ( ; indexIter != indexEnd; indexIter++, colorIter++) {
					...
					EM_COUT_D("Vertex " << (*shapeIter)->m_vVtxAlign[(*indexIter)].x << " " 
									<< (*shapeIter)->m_vVtxAlign[(*indexIter)].y << " " 
									<< (*shapeIter)->m_vVtxAlign[(*indexIter)].z << endl, 1);
				}
				glEnd();
#endif
			}
	...
	case EM_GL_GCOL_TRANS: {
		...
		for ( ; shapeIter != shapeEnd; shapeIter++) {
			...
			EM_COUT_D("OpenGLTransVisitor::visit() "<< (*shapeIter)->m_vPolygon.size() <<" polygons"<< endl, 0);
			...
			for ( ; polyIter != polyEnd; polyIter++) {
				...
				EmAssert((*polyIter)->m_vIndex.size() == (*polyIter)->m_vColor.size(), "size miss match");
				glBegin(GL_POLYGON);
				for ( ; indexIter != indexEnd; indexIter++, colorIter++) {

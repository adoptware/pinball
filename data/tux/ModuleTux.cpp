...
public:
  ...
  void StdOnSignal()
  {
	...
	if (m_aBoot[a] != m_aBoot[a+1])
	{
	  ...
	  if (m_aBoot[a])
	    SendSignal(m_sigBootOn[a], 0, this->getParent(), NULL);
	  else
	    SendSignal(m_sigBootOff[a], 0, this->getParent(), NULL);
	} 
      } 
      if (m_aBoot[3] != tmp)
      {
	...
	if (m_aLinux[aa] != m_aLinux[aa+1])
	{
	  ...
	  if (m_aLinux[aa])
	    SendSignal(m_sigLinuxOn[aa], 0, this->getParent(), NULL);
	  else
	    SendSignal(m_sigLinuxOff[aa], 0, this->getParent(), NULL);
	}
      } 
      if (m_aLinux[4] != tmp)
      {
	...
	for (int a=4; a>0; --a)
	{
	  if (m_aLinux[a] != m_aLinux[a-1])
	  {
	    ...
	    if (m_aLinux[a])
	      SendSignal(m_sigLinuxOn[a], 0, this->getParent(), NULL);
	    else
	      SendSignal(m_sigLinuxOff[a], 0, this->getParent(), NULL);
	  }
	}
      }
      if (m_aLinux[0] != tmp)
    ...
    // LINUX all
    if (m_aLinux[0] && m_aLinux[1] && m_aLinux[2] &&
	m_aLinux[3] && m_aLinux[4])
    {
      SendSignal(m_sigLinuxAll, 0, this->getParent(), NULL);
      m_aLinux[0] = m_aLinux[1] = m_aLinux[2] = false;
      m_aLinux[3] = m_aLinux[4] = false;

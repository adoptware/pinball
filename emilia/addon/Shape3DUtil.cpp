...
Shape3D* Shape3DUtil::loadShape3D(const char* fileName) {
	...
	try {
		...
		while(file) {
			if (str == "shp") {
				EM_COUT("Shape3DUtil::load() : loading shape", 1);
				shape = new Shape3D();
...
Shape3D* Shape3DUtil::loadShape3Dold(const char* fileName) {
  ...
  for (a=0; a<tiPolygons; a++) {
	...
	EM_COUT("Shape3DUtil::loadShape3D() Done." << endl, 1);
	return toShape3D;
}

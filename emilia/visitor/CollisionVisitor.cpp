...
bool CollisionVisitor::detectCollision(CollisionBounds * cb1, CollisionBounds * cb2, Vertex3D & n1, Vertex3D & n2) {
	...
	if (this->collide(cb1, cb2)) {
		...
		if (cb1->m_vCollisionBounds.size() > 0 && cb2->m_vCollisionBounds.size() > 0) {
			...
			for ( ; iter1 != end1; iter1++) {
				vector<CollisionBounds*>::iterator iter2 = cb1->m_vCollisionBounds.begin();
				vector<CollisionBounds*>::iterator end2 = cb1->m_vCollisionBounds.end();
				for ( ; iter2 != end2; iter2++) {
		...
		// cb1 and cb2 are leaves
		 	...
		 	if (this->collidePolygons(cb1, cb2)) {
				...
				EM_COUT("CollisionVisitor::detectCollision() A "<< m_iPolygonsA <<" polys "<< n1.x <<" "<< n1.y <<" "<< n1.z, 0);
				EM_COUT("CollisionVisitor::detectCollision() B "<< m_iPolygonsB <<" polys "<< n2.x <<" "<< n2.y <<" "<< n2.z, 0);
				return true;
			}
...
/* Do not detect collisions if both groups are without behavior.
 */
void CollisionVisitor::visit(Group * g) {	
	...
	vector<Group*>::iterator end = m_vGroup.end();
	for ( ; iter != end; iter++) {
		m_iPolygonsA = 0;
		m_iPolygonsB = 0;
//		EM_COUT("CollisionVisitor::visit() visit " << aGroups[a]->iUserProperties << endl, 0);
		if ((*iter)->m_iUserProperties & g->m_iUserProperties) {
//			EM_COUT("CollisionVisitor::visit() same group " << endl, 0);
			continue;
		}
		if ((*iter)->m_vBehavior.size() == 0 && g->m_vBehavior.size() == 0) continue;
		if (this->detectCollision((*iter)->p_CollisionBounds, g->p_CollisionBounds, 
															vtxNormal1, vtxNormal2)) {
			EM_COUT("CollisionVisitor::visit() collision!" << endl, 0);
